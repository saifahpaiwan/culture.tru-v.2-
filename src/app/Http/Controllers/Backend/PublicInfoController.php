...
class PublicInfoController extends Controller
{
    ...
    {
        $data = array();
        return view('backend.publicinfo.list', compact('data'));
    }

    public function publicinfoadd()
    {
        $data = array();
        return view('backend.publicinfo.add', compact('data'));
    }

    public function publicinfoedit($get_id)
    {
        $data = array(
            "publicinfo_find" => publicinfo::find($get_id),
        );
 
        return view('backend.publicinfo.edit', compact('data'));
    }

    public function publicinfoDropzone($get_id)
    {
        $data = array(
            "publicinfo_find" => publicinfo::find($get_id),
            "gallerys"      => DB::table('publicinfo_gallerys')->where('publicinfo_id', $get_id)->get(),
            "get_id"        => $get_id,
        );
        return view('backend.publicinfo.dropzone', compact('data'));
    }
    ...
    public function Query_Datatable($keywrod, $status, $date)
    {
        ...
        $date_sql = "";
        if (isset($keywrod)) {
            $keywrod_sql = " and publicinfo.title LIKE '%" . $keywrod . "%'";
        }

        if (isset($status)) {
            $status_sql = " and publicinfo.deleted_at = " . $status . "";
        }

        if (isset($date)) {
            $date_sql = " and (publicinfo.created_at BETWEEN '" . $date . " 00:00:00' AND '" . $date . " 23:59:59')";
        }
        ...
        order by publicinfo.id DESC');

        return $data;
    }

    public function datatablepublicinfo(Request $request)
    {
        if ($request->ajax()) {
            // ===================QUERY-DATATABLE======================= //
            ...
            return Datatables::of($data)
                ...
                ->addColumn('images', function ($row) {
                    $img = '<img src="' . asset('images/publicinfo') . '/' . $row->image_desktop . '" alt="contact-img" title="contact-img" class="mr-2" width="100%" style="border: 1px solid #ddd;border-radius: 5px;">';
                    return '<a href="' . route('publicinfo.edit', [$row->id]) . '"> ' . $img . ' </a>';
                })
                ->addColumn('title', function ($row) {
                    return '<a href="' . route('publicinfo.edit', [$row->id]) . '"> '  . $row->title . '</a>';
                })
                ...
                ->addColumn('bntManger', function ($row) {
                    $html = '<a href="' . route('publicinfo.dropzone', [$row->id]) . '" class="btn btn-xs btn-icon waves-effect btn-primary mb-1"> <i class="fe-image"></i> </a>';
                    $html .= '<a href="' . route('publicinfo.edit', [$row->id]) . '" class="btn btn-xs btn-icon waves-effect btn-secondary mb-1"> <i class="mdi mdi-pencil"></i> </a>';
    ...
    public function closepublicinfo(Request $request)
    {
        if (isset($request)) {
            $data = publicinfo::find($request->id);
            $uploade_location = 'images/publicinfo/';
            $uploade_location_pdf = 'images/publicinfo/pdf/';

            $data_gallerys = DB::table('publicinfo_gallerys')->where('publicinfo_gallerys.publicinfo_id', $request->id)->get();
            if (isset($data_gallerys) && count($data_gallerys) > 0) {
                $uploade_location_gallery = 'images/publicinfo/gallery/';
            ...
            $data = DB::table('publicinfo')
                ->where('publicinfo.id', $request->id)
                ->delete();
            $data_gallerys = DB::table('publicinfo_gallerys')
                ->where('publicinfo_gallerys.publicinfo_id', $request->id)
                ->delete();
        }
        return $data;
    }

    public function savePublicinfo(Request $request)
    {
        if (isset($request)) {
            $request->validate(
                [
            ...
            if ($request->statusData == "C") {
                ...
                $file_name_pdf = NULL;
                $file_text = "publicinfo-" . hexdec(uniqid()) . ".text";
            ...
            if ($request->file('file_upload_dektop')) {
                if (!empty($request->file('file_upload_dektop'))) {
                    $uploade_location_dektop = 'images/publicinfo/';
            ...
            if ($request->statusData == "C") {
                DB::table('publicinfo')->insert($data);
                return redirect()->route('publicinfo.add')->with('success', $msg);
            } else if ($request->statusData == "U") {
                DB::table('publicinfo')
                    ->where('publicinfo.id', $request->id)
                    ->update($data);
                return redirect()->route('publicinfo.edit', [$request->id])->with('success', $msg);
            }
        }
    }

    public function closepublicinfoPdf(Request $request)
    {
        if (isset($request)) {
            $uploade_location_pdf = 'images/publicinfo/pdf/';
    ...
    public function savePublicinfoDropzone(Request $request)
    { 
         if ($request->hasFile('file')) {
            ...
            $destinationPath = public_path('images/publicinfo/gallery/');
            ...
            if (!empty($filename)) {
                $data = array(
                    'publicinfo_id'   => $request->id,
                    'image_desktop' => $filename,
                    'ip_address' => $request->ip(),
                    'created_at' => new \DateTime(),
                );
            }
            return DB::table('publicinfo_gallerys')->insert($data);
        }

        return response()->json(['error' => 'No file uploaded.'], 400);
    }

    public function closepublicinfogallery(Request $request)
    {
        if (isset($request)) {
            $data = publicinfo_gallerys::find($request->id);
            $uploade_location = 'images/publicinfo/gallery/';
            if (!empty($data->image_desktop)) {
                @unlink($uploade_location . $data->image_desktop);
            }

            $data = DB::table('publicinfo_gallerys')
                ->where('publicinfo_gallerys.id', $request->id)
                ->delete();
        }
        return $data;
    }

    public function closePublicInfoDropzoneAll(Request $request)
    {
        if (isset($request)) {
            $data = publicinfo_gallerys::where('publicinfo_id', $request->id)->get();
            $uploade_location = 'images/publicinfo/gallery/';
        ...
        $data = DB::table('publicinfo_gallerys')->where('publicinfo_id', $request->id)->delete();
        return $data;
    }
}
