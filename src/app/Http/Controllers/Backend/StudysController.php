...
{
    public function studyslist()
    {
        $data = array();
        return view('backend.studys.list', compact('data'));
    }

    public function studysadd()
    {
        $data = array();
        return view('backend.studys.add', compact('data'));
    }

    public function studysedit($get_id)
    {
        $data = array(
            "studys_find" => studies::find($get_id),
        );
        return view('backend.studys.edit', compact('data'));
    }

    public function studysdropzone($get_id)
    {
        $data = array(
            "studys_find" => studies::find($get_id),
            "gallerys"      => DB::table('study_gallerys')->where('study_id', $get_id)->get(),
            "get_id"        => $get_id,
        );
        return view('backend.studys.dropzone', compact('data'));
    }
    ...
    public function Query_Datatable($keywrod, $status, $date)
    {
        ...
        $date_sql = "";
        if (isset($keywrod)) {
            $keywrod_sql = " and studies.title LIKE '%" . $keywrod . "%'";
        }

        if (isset($status)) {
            $status_sql = " and studies.deleted_at = " . $status . "";
        }

        if (isset($date)) {
            $date_sql = " and (studies.created_at BETWEEN '" . $date . " 00:00:00' AND '" . $date . " 23:59:59')";
        }
    ...
    public function datatablestudys(Request $request)
    {
        if ($request->ajax()) {
            // ===================QUERY-DATATABLE======================= //
            ...
            return Datatables::of($data)
                ...
                ->addColumn('images', function ($row) {
                    $img = '<img src="' . asset('images/studys') . '/' . $row->image_desktop . '" alt="contact-img" title="contact-img" class="mr-2" width="100%" style="border: 1px solid #ddd;border-radius: 5px;">';
                    return '<a href="' . route('studys.edit', [$row->id]) . '"> ' . $img . ' </a>';
                })
                ->addColumn('title', function ($row) {
                    return '<a href="' . route('studys.edit', [$row->id]) . '"> ' .  $row->title . '</a>';
                })
                ...
                ->addColumn('bntManger', function ($row) {
                    $html = '<a href="' . route('studys.dropzone', [$row->id]) . '" class="btn btn-xs btn-icon waves-effect btn-primary mb-1"> <i class="fe-image"></i> </a>';
                    $html .= '<a href="' . route('studys.edit', [$row->id]) . '" class="btn btn-xs btn-icon waves-effect btn-secondary mb-1"> <i class="mdi mdi-pencil"></i> </a>';
    ...
    public function closestudys(Request $request)
    {
        if (isset($request)) {
            $data = studies::find($request->id);
            $uploade_location = 'images/studys/';
            $uploade_location_pdf = 'images/studys/pdf/';

            $data_gallerys = DB::table('study_gallerys')->where('study_gallerys.study_id', $request->id)->get();
            if (isset($data_gallerys) && count($data_gallerys) > 0) {
                $uploade_location_gallery = 'images/studys/gallery/';
            ...
            $data = DB::table('studies')
                ->where('studies.id', $request->id)
                ->delete();
            $data_gallerys = DB::table('study_gallerys')
                ->where('study_gallerys.study_id', $request->id)
                ->delete();
        }
        return $data;
    }

    public function savestudys(Request $request)
    {
        if (isset($request)) {
            $request->validate(
                [
            ...
            if ($request->statusData == "C") {
                ...
                $file_name_pdf = NULL;
                $file_text = "studys-" . hexdec(uniqid()) . ".text";
            ...
            if ($request->file('file_upload_dektop')) {
                if (!empty($request->file('file_upload_dektop'))) {
                    $uploade_location_dektop = 'images/studys/';
            ...
            if ($request->statusData == "C") {
                ...
                return redirect()->route('studys.add')->with('success', $msg);
            } else if ($request->statusData == "U") {
                DB::table('studies')
                    ->where('studies.id', $request->id)
                    ->update($data);
                return redirect()->route('studys.edit', [$request->id])->with('success', $msg);
            }
        }
    }

    public function closestudysPdf(Request $request)
    {
        if (isset($request)) {
            $uploade_location_pdf = 'images/studys/pdf/';
    ...
    public function savestudysDropzone(Request $request)
    {
        if ($request->hasFile('file')) {
            ...
            $destinationPath = public_path('images/studys/gallery/');
            ...
            return DB::table('study_gallerys')->insert($data);
        }

        return response()->json(['error' => 'No file uploaded.'], 400);
    }

    public function closestudysgallery(Request $request)
    {
        if (isset($request)) {
            $data = study_gallerys::find($request->id);
            $uploade_location = 'images/studys/gallery/';
            if (!empty($data->image_desktop)) {
                @unlink($uploade_location . $data->image_desktop);
            }

            $data = DB::table('study_gallerys')
                ->where('study_gallerys.id', $request->id)
                ->delete();
        }
        return $data;
    }

    public function closeStudysDropzoneAll(Request $request)
    {
        if (isset($request)) {
            $data = study_gallerys::where('study_id', $request->id)->get();
            $uploade_location = 'images/studys/gallery/';
        ...
        $data = DB::table('study_gallerys')->where('study_id', $request->id)->delete();
        return $data;
    }
}
