...
use App\Models\research_gallerys;
...
class ResearchController extends Controller
{
    ...
    {
        $data = array();
        return view('backend.research.list', compact('data'));
    }

    public function researchadd()
    {
        $data = array();
        return view('backend.research.add', compact('data'));
    }

    public function researchedit($get_id)
    {
        $data = array(
            "research_find" => researchs::find($get_id),
        );
        return view('backend.research.edit', compact('data'));
    }

    public function researchdropzone($get_id)
    {
        $data = array(
            "research_find" => researchs::find($get_id),
        ...
        return view('backend.research.dropzone', compact('data'));
    }
    ...
    public function Query_Datatable($keywrod, $status, $date)
    {
        ...
        if (isset($keywrod)) {
            $keywrod_sql = " and researchs.title LIKE '%" . $keywrod . "%'
            or researchs.author LIKE '%" . $keywrod . "%' ";
        }

        if (isset($status)) {
            $status_sql = " and researchs.deleted_at = " . $status . "";
        }

        if (isset($date)) {
            $date_sql = " and researchs.year LIKE '%" . $date . "%'";
        }
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                ...
                ->addColumn('images', function ($row) {
                    $img = '<img src="' . asset('images/research') . '/' . $row->image_desktop . '" alt="contact-img" title="contact-img" class="mr-2" width="100%" style="border: 1px solid #ddd;border-radius: 5px;">';
                    return '<a href="' . route('research.edit', [$row->id]) . '"> ' . $img . ' </a>';
                })
                ->addColumn('title', function ($row) {
                    return '<a href="' . route('research.edit', [$row->id]) . '"> ' . $row->title . '</a>';
                })
                ->addColumn('author', function ($row) {
                    return $row->author;
                })
                ->addColumn('type', function ($row) {
                    return ($row->type == 1) ? "วิจัย" : "บทความ";
                })
                ...
                ->addColumn('bntManger', function ($row) {
                    $html = '<a href="' . route('research.dropzone', [$row->id]) . '" class="btn btn-xs btn-icon waves-effect btn-primary mb-1"> <i class="fe-image"></i> </a>';
                    $html .= '<a href="' . route('research.edit', [$row->id]) . '" class="btn btn-xs btn-icon waves-effect btn-secondary mb-1"> <i class="mdi mdi-pencil"></i> </a>';
    ...
    {
        if (isset($request)) {
            $data = researchs::find($request->id);
            $uploade_location = 'images/research/';
            $uploade_location_pdf = 'images/research/pdf/';

            $data_gallerys = DB::table('research_gallerys')->where('research_gallerys.research_id', $request->id)->get();
            if (isset($data_gallerys) && count($data_gallerys) > 0) {
                $uploade_location_gallery = 'images/research/gallery/';
            ...
            $data = DB::table('researchs')
                ->where('researchs.id', $request->id)
                ->delete();
            $data_gallerys = DB::table('research_gallerys')
                ->where('research_gallerys.research_id', $request->id)
                ->delete();
        }
        return $data;
    }
    ...
    public function saveresearch(Request $request)
    {
        if (isset($request)) {
            ...
            if ($request->statusData == "C") {
                ...
                $file_name_pdf = NULL;
                $file_text = "research-" . hexdec(uniqid()) . ".text";
            ...
            if ($request->statusData == "C") {
                DB::table('researchs')->insert($data);
    ...
    public function saveresearchDropzone(Request $request)
    { 
        if ($request->hasFile('file')) {
            ...
            $destinationPath = public_path('images/research/gallery/');
            ...
            return DB::table('research_gallerys')->insert($data);
        }

        return response()->json(['error' => 'No file uploaded.'], 400);
    }

    public function closeresearchgallery(Request $request)
    {
        if (isset($request)) {
            $data = research_gallerys::find($request->id);
            $uploade_location = 'images/research/gallery/';
            if (!empty($data->image_desktop)) {
                @unlink($uploade_location . $data->image_desktop);
            }

            $data = DB::table('research_gallerys')
                ->where('research_gallerys.id', $request->id)
                ->delete();
        }
        return $data;
    }

    public function closeResearchDropzoneAll(Request $request)
    {
        if (isset($request)) {
            $data = research_gallerys::where('research_id', $request->id)->get();
            $uploade_location = 'images/research/gallery/';
        ...
        $data = DB::table('research_gallerys')->where('research_id', $request->id)->delete();
        return $data;
    }
}
