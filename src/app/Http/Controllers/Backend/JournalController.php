...
class JournalController extends Controller
{
    ...
    {
        $data = array();
        return view('backend.journals.list', compact('data'));
    }

    public function journalsadd()
    {
        $data = array();
        return view('backend.journals.add', compact('data'));
    }

    public function journalsedit($get_id)
    {
        $data = array(
            "journals_find" => journals::find($get_id),
        );
        return view('backend.journals.edit', compact('data'));
    }

    public function journalsdropzone($get_id)
    {
        $data = array(
            "journals_find" => journals::find($get_id),
            "gallerys"      => DB::table('journal_gallerys')->where('journal_id', $get_id)->get(),
            "get_id"        => $get_id,
        );
        return view('backend.journals.dropzone', compact('data'));
    }
    ...
    public function Query_Datatable($keywrod, $status, $date)
    {
        ...
        $date_sql = "";
        if (isset($keywrod)) {
            $keywrod_sql = " and journals.title LIKE '%" . $keywrod . "%'";
        }

        if (isset($status)) {
            $status_sql = " and journals.deleted_at = " . $status . "";
        }

        if (isset($date)) {
            $date_sql = " and (journals.created_at BETWEEN '" . $date . " 00:00:00' AND '" . $date . " 23:59:59')";
        }
    ...
    public function datatablejournals(Request $request)
    {
        if ($request->ajax()) {
            // ===================QUERY-DATATABLE======================= //
            ...
            return Datatables::of($data)
                ...
                ->addColumn('images', function ($row) {
                    $img = '<img src="' . asset('images/journals') . '/' . $row->image_desktop . '" alt="contact-img" title="contact-img" class="mr-2" width="100%" style="border: 1px solid #ddd;border-radius: 5px;">';
                    return '<a href="' . route('journals.edit', [$row->id]) . '"> ' . $img . ' </a>';
                })
                ->addColumn('title', function ($row) { 
                    return '<a href="' . route('journals.edit', [$row->id]) . '"> ' . $row->title . '</a>';
                })
                ...
                ->addColumn('bntManger', function ($row) {
                    $html = '<a href="' . route('journals.dropzone', [$row->id]) . '" class="btn btn-xs btn-icon waves-effect btn-primary mb-1"> <i class="fe-image"></i> </a>';
                    $html .= '<a href="' . route('journals.edit', [$row->id]) . '" class="btn btn-xs btn-icon waves-effect btn-secondary mb-1"> <i class="mdi mdi-pencil"></i> </a>';
    ...
    public function closejournals(Request $request)
    {
        if (isset($request)) {
            $data = journals::find($request->id);
            $uploade_location = 'images/journals/';
            $uploade_location_pdf = 'images/journals/pdf/';

            $data_gallerys = DB::table('journal_gallerys')->where('journal_gallerys.journal_id', $request->id)->get();
            if (isset($data_gallerys) && count($data_gallerys) > 0) {
                $uploade_location_gallery = 'images/journals/gallery/';
            ...
            $data = DB::table('journals')
                ->where('journals.id', $request->id)
                ->delete();
            $data_gallerys = DB::table('journal_gallerys')
                ->where('journal_gallerys.journal_id', $request->id)
                ->delete();
        }
        return $data;
    }
    ...
    public function savejournals(Request $request)
    {
        if (isset($request)) {
            ...
            if ($request->statusData == "C") {
                ...
                $file_name_pdf = NULL;
                $file_text = "journals-" . hexdec(uniqid()) . ".text";
            ...
            if ($request->file('file_upload_dektop')) {
                if (!empty($request->file('file_upload_dektop'))) {
                    $uploade_location_dektop = 'images/journals/';
            ...
            if ($request->statusData == "C") {
                ...
                return redirect()->route('journals.add')->with('success', $msg);
            } else if ($request->statusData == "U") {
                DB::table('journals')
                    ->where('journals.id', $request->id)
                    ->update($data);
                return redirect()->route('journals.edit', [$request->id])->with('success', $msg);
            }
        }
    }

    public function closejournalsPdf(Request $request)
    {
        if (isset($request)) {
            $uploade_location_pdf = 'images/journals/pdf/';
    ...
    public function savejournalsDropzone(Request $request)
    { 
        if ($request->hasFile('file')) {
            ...
            $destinationPath = public_path('images/journals/gallery/');
            ...
            return DB::table('journal_gallerys')->insert($data);
        }

        return response()->json(['error' => 'No file uploaded.'], 400);
    }

    public function closejournalsgallery(Request $request)
    {
        if (isset($request)) {
            $data = journal_gallerys::find($request->id);
            $uploade_location = 'images/journals/gallery/';
            if (!empty($data->image_desktop)) {
               @unlink($uploade_location . $data->image_desktop);
            }

            $data = DB::table('journal_gallerys')
                ->where('journal_gallerys.id', $request->id)
                ->delete();
        }
        return $data;
    }

    public function closeJournalsDropzoneAll(Request $request)
    {
        if (isset($request)) {
            $data = journal_gallerys::where('journal_id', $request->id)->get();
            $uploade_location = 'images/journals/gallery/';
        ...
        $data = DB::table('journal_gallerys')->where('journal_id', $request->id)->delete();
        return $data;
    }
}
