...
use App\Models\acticonservation_gallerys;
use Illuminate\Support\Facades\Storage;
use Intervention\Image\Facades\Image;

class ActiconservationController extends Controller
{
    public function acticonservationlist()
    {
        $data = array();
        return view('backend.acticonservation.list', compact('data'));
    }

    public function acticonservationadd()
    {
        $data = array();
        return view('backend.acticonservation.add', compact('data'));
    }

    public function acticonservationedit($get_id)
    {
        $data = array(
            "acticonservation_find" => acticonservation::find($get_id),
        );
        return view('backend.acticonservation.edit', compact('data'));
    }

    public function acticonservationdropzone($get_id)
    {
        $data = array(
            "acticonservation_find" => acticonservation::find($get_id),
        ...
        return view('backend.acticonservation.dropzone', compact('data'));
    }
    ...
    public function Query_Datatable($keywrod, $status, $date)
    {
        ...
        $date_sql = "";
        if (isset($keywrod)) {
            $keywrod_sql = " and acticonservations.title LIKE '%" . $keywrod . "%'";
        }

        if (isset($status)) {
            $status_sql = " and acticonservations.deleted_at = " . $status . "";
        }

        if (isset($date)) {
            $date_sql = " and (acticonservations.created_at BETWEEN '" . $date . " 00:00:00' AND '" . $date . " 23:59:59')";
        }

        $data = DB::select('select * 
        from `acticonservations` 
        where acticonservations.id != "" 
        ' . $keywrod_sql . ' ' . $status_sql . ' ' . $date_sql . '
        order by acticonservations.id DESC');
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                ...
                ->addColumn('images', function ($row) {
                    $img = '<img src="' . asset('images/acticonservation') . '/' . $row->image_desktop . '" alt="contact-img" title="contact-img" class="mr-2" width="100%" style="border: 1px solid #ddd;border-radius: 5px;">';
                    return '<a href="' . route('acticonservation.edit', [$row->id]) . '"> ' . $img . ' </a>';
                })
                ->addColumn('title', function ($row) {
                    return '<a href="' . route('acticonservation.edit', [$row->id]) . '"> ' . $row->title . '</a>';
                })
                ...
                ->addColumn('bntManger', function ($row) {
                    $html = '<a href="' . route('acticonservation.dropzone', [$row->id]) . '" class="btn btn-xs btn-icon waves-effect btn-primary mb-1"> <i class="fe-image"></i> </a>';
                    $html .= '<a href="' . route('acticonservation.edit', [$row->id]) . '" class="btn btn-xs btn-icon waves-effect btn-secondary mb-1"> <i class="mdi mdi-pencil"></i> </a>';
    ...
    {
        if (isset($request)) {
            $data = acticonservation::find($request->id);
            $uploade_location = 'images/acticonservation/';
            $uploade_location_pdf = 'images/acticonservation/pdf/';

            $data_gallerys = DB::table('acticonservation_gallerys')->where('acticonservation_gallerys.acticonservation_id', $request->id)->get();
            if (isset($data_gallerys) && count($data_gallerys) > 0) {
                $uploade_location_gallery = 'images/acticonservation/gallery/';
            ...
            $data = DB::table('acticonservations')
                ->where('acticonservations.id', $request->id)
                ->delete();
            $data_gallerys = DB::table('acticonservation_gallerys')
                ->where('acticonservation_gallerys.acticonservation_id', $request->id)
                ->delete();
        }
        return $data;
    }
    ...
    public function saveacticonservation(Request $request)
    {
        if (isset($request)) {
            ...
            if ($request->statusData == "C") {
                DB::table('acticonservations')->insert($data);
                return redirect()->route('acticonservation.add')->with('success', $msg);
            } else if ($request->statusData == "U") {
                DB::table('acticonservations')
                    ->where('acticonservations.id', $request->id)
                    ->update($data);
    ...
    public function saveacticonservationDropzone(Request $request)
    { 
        if ($request->hasFile('file')) {
            ...
            $destinationPath = public_path('images/acticonservation/gallery/');
            ...
            if (!empty($filename)) {
                $data = array(
                    'acticonservation_id'   => $request->id,
                    'image_desktop' => $filename,
                    'ip_address' => $request->ip(),
                    'created_at' => new \DateTime(),
                );
            }
            return DB::table('acticonservation_gallerys')->insert($data);
        }

        return response()->json(['error' => 'No file uploaded.'], 400);
    }

    public function closeacticonservationgallery(Request $request)
    {
        if (isset($request)) {
            $data = acticonservation_gallerys::find($request->id);
            $uploade_location = 'images/acticonservation/gallery/';
            if (!empty($data->image_desktop)) {
                @unlink($uploade_location . $data->image_desktop);
            }

            $data = DB::table('acticonservation_gallerys')
                ->where('acticonservation_gallerys.id', $request->id)
                ->delete();
        }
        return $data;
    }

    public function closeActiconservationDropzoneAll(Request $request)
    {
        if (isset($request)) {
            $data = acticonservation_gallerys::where('acticonservation_id', $request->id)->get();
            $uploade_location = 'images/acticonservation/gallery/';
        ...
        $data = DB::table('acticonservation_gallerys')->where('acticonservation_id', $request->id)->delete();
        return $data;
    }
}
