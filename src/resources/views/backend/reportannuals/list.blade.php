          ...
          <div class="card-header" style="background: #ddd;">
            <div class="row">
              ...
              <div class="col-md-3 text-right">
                ...
                <a href="{{ route('reportannuals.add') }}" class="btn btn-primary waves-effect waves-light"><i class="fe-plus-square"></i> เพิ่ม </a>
              </div>
            </div>
          </div>
          <div class="card-body">
              ...
              <table class="table table-borderless nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;" id="tbl-data">
                <thead>
                  <tr>
                    <th width="5%"> #รหัส </th>
                    <th width="15%"> รูป </th>
                    <th> ชื่อประกันคุณภาพการศึกษา </th>
...
<script>
  ...
  function datatable(keywrod, status, date) {
    var table = $('#tbl-data').DataTable({
      ...
      "order": [],
      ajax: {
        url: "{{ route('datatable.reportannuals') }}",
        data: {
          keywrod: keywrod,
          status: status,
          date: date,
