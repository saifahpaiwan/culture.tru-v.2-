    ...
    <div class="row">
      <div class="col-12">
        <div class="page-title-box">
          <h4 class="page-title"> <i class="dripicons-broadcast"></i> จัดการข่าว </h4>
        </div>
      </div>
    </div>
        ...
        <div class="card">
          <div class="card-header" style="background: #ddd;">
            <div class="row align-items-center">
              <div class="col-md-6">
                <a href="{{ route('news.list') }}" class="btn btn-xs btn-icon waves-effect btn-secondary mิ-1"><i class="fe-chevron-left"></i> ย้อนกลับ </a>
              </div>
              <div class="col-md-6 text-right">
                <a href="{{ route('news.dropzone', $data['get_id']) }}" class="btn btn-xs btn-icon waves-effect btn-dark mิ-1"><i class="fe-rotate-cw"></i> รีเซ็ต </a>
          ...
          <div class="card-body">
            <h5>ประมวณภาพ : {{ $data['news_find']->news_title }} </h5>
            <p> {{ $data['news_find']->news_intro }} </p>
            <div>
              <form action="{{ route('save.news.dropzone') }}" method="POST" class="dropzone dz-clickable" id="myAwesomeDropzone" enctype="multipart/form-data">
                @csrf
            ...
            <div class="preview mt-2">
              ...
              <div class="d-flex flex-wrap">
                ...
                <div class="mb-2">
                  <a href="javascript:void(0)" class="preview-images" data-img="{{ asset('images/news/gallery').'/'.$row->image_desktop }}">
                    <div class="box-img-gallery" style="background: url(<?php echo asset('images/news/gallery') . '/' . $row->image_desktop; ?>); background-size: cover; background-position: center;"></div>
                  </a>
...
paramName: "file",
  ...
  $(document).on('click', '.remove', function(event) {
    ...
    Swal.fire({
      ...
      if (result.value) {
        $.post("{{ route('close.news.gallery') }}", {
            _token: "{{ csrf_token() }}",
            id: id,
          })
          .done(function(data, status, error) {
            if (error.status == 200) {
              location.href = "{{ route('news.dropzone', [$data['get_id']]) }}";
            }
          })
  ...
  $(document).on('click', '.delete-all', function(event) {
    ...
    Swal.fire({
      ...
      if (result.value) {
        $.post("{{ route('close.news.dropzone.all') }}", {
            _token: "{{ csrf_token() }}",
            id: id,
          })
          .done(function(data, status, error) {
