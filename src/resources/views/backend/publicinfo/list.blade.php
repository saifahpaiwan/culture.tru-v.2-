          ...
          <div class="card-header" style="background: #ddd;">
            <div class="row">
              ...
              <div class="col-md-3 text-right">
                ...
                <a href="{{ route('publicinfo.add') }}" class="btn btn-primary waves-effect waves-light"><i class="fe-plus-square"></i> เพิ่ม </a>
              </div>
            </div>
          </div>
          <div class="card-body">
...
<script>
  ...
  function datatable(keywrod, status, date) {
    var table = $('#tbl-data').DataTable({
      ...
      "order": [],
      ajax: {
        url: "{{ route('datatable.publicinfo') }}",
        data: {
          keywrod: keywrod,
          status: status,
          date: date,
