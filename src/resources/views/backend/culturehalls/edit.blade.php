      ...
      <div class="col-lg-12">
        <div class="card">
          <div class="card-header" style="background: #ddd;">
            <div class="row">
              <div class="col-md-12">
                <h5 class="m-0"> แก้ไขหอวัฒนธรรม
                  <a href="{{ route('culturehalls.list') }}" class="float-right"><i class="fe-chevron-left"></i> ย้อนกลับ </a>
                </h5>
              </div>
            </div>
          </div>
          <div class="card-body">
            ...
            @endif
            <form method="POST" action="{{ route('save.culturehalls') }}" id="form" enctype="multipart/form-data">
              @csrf
              <input type="hidden" id="statusData" name="statusData" value="U">
              <input type="hidden" id="id" name="id" value="{{ $data['culturehalls_find']->id }}">

              <div class="row">
                <div class="col-md-8">
                  <div class="row">
                    <div class="col-md-12 form-group">
                      <label class="ml-1" for="title"> ชื่อหอวัฒนธรรม <span class="text-danger">*</span></label>
                      <input id="title" type="text" class="form-control form-control-lg @error('title') invalid @enderror" name="title" value="{{ $data['culturehalls_find']->title }}" required autocomplete="title" autofocus placeholder="โปรดระบุข้อมูล...">
                      @error('title')
                    ...
                    <div class="col-md-12 form-group">
                      ...
                      <textarea class="form-control form-control-lg @error('intro') invalid @enderror" rows="2" id="intro" name="intro" autocomplete="intro" autofocus placeholder="โปรดระบุข้อมูล..." required>{{ $data['culturehalls_find']->intro }}</textarea>
                      @error('intro')
                      <ul class="parsley-errors-list filled">
                        <li class="parsley-required">{{ $message }}</li>
                      </ul>
                    ...
                    <div class="col-md-12 form-group">
                      ...
                      <input id="meta_title" type="text" class="form-control form-control-lg @error('meta_title') invalid @enderror" name="meta_title" value="{{ $data['culturehalls_find']->meta_title }}" autocomplete="meta_title" autofocus placeholder="โปรดระบุข้อมูล...">
                      @error('meta_title')
                      <ul class="parsley-errors-list filled">
                        <li class="parsley-required">{{ $message }}</li>
                      </ul>
                    ...
                    <div class="col-md-12 form-group">
                      ...
                      <input id="meta_description" type="text" class="form-control form-control-lg @error('meta_description') invalid @enderror" name="meta_description" value="{{ $data['culturehalls_find']->meta_description }}" autocomplete="meta_description" autofocus placeholder="โปรดระบุข้อมูล...">
                    ...
                    <div class="col-md-12 form-group">
                      ...
                      <input id="meta_keyword" type="text" class="form-control form-control-lg @error('meta_keyword') invalid @enderror" name="meta_keyword" value="{{ $data['culturehalls_find']->meta_keyword }}" autocomplete="meta_keyword" autofocus placeholder="โปรดระบุข้อมูล...">
                      @error('meta_keyword')
                      <ul class="parsley-errors-list filled">
                        <li class="parsley-required">{{ $message }}</li>
                      </ul>
                  ...
                  <div class="col-md-12 form-group">
                    <div class="mb-1">
                      ...
                      <div class="img-file-upload-2">
                        <img src="{{ asset('images/culturehalls').'/'.$data['culturehalls_find']->image_desktop }}" class="rounded w-100">
                      </div>
                      ...
                      <input type="hidden" id="file_upload_dektop_hdf" name="file_upload_dektop_hdf" value="{{ $data['culturehalls_find']->image_desktop }}">
                  ...
                  <div class="col-md-12 form-group">
                    <div class="mb-2">
                      ...
                      <div class="cc-selector">
                        <input id="status1" type="radio" name="status" value="0" {{  $data['culturehalls_find']->deleted_at == true ?  $data['culturehalls_find']->deleted_at == 0 ? "checked" : ""  : "checked"  }} />
                        <label class="drinkcard-cc bg-success" for="status1"> เปิดการแสดงผล </label>

                        <input id="status2" type="radio" name="status" value="1" {{  $data['culturehalls_find']->deleted_at == 1 ? "checked" : "" }} />
                    ...
                    <input type="hidden" id="file_upload_pdf_hdf" name="file_upload_pdf_hdf" value="{{ $data['culturehalls_find']->file_pdf }}">
                    @if(!empty($data['culturehalls_find']->file_pdf))
                    <a href="javascript: void(0);" data-file="{{ $data['culturehalls_find']->file_pdf }}" class="h2 m-0 text-danger" id="close-pdf"><i class="fe-trash-2"></i></a>
                    @endif

                    <span class="mt-1 ml-1" style="font-size: 10px;"> {{ $data['culturehalls_find']->file_pdf }} </span>
                ...
                <div class="col-md-12">
                  <hr>
                  <label for="file_text">รายละเอียดหอวัฒนธรรม <span class="text-danger">*</span></label>
                  <textarea cols="10" id="file_text" name="file_text" rows="10"><?php echo @file_get_contents(storage_path() . '/app/' . $data['culturehalls_find']->file_text); ?></textarea>
                  <input type="hidden" id="file_text_hdfs" name="file_text_hdfs" value="{{ $data['culturehalls_find']->file_text }}">
                  @error('file_text')
                  <span class="invalid-feedback" role="alert">
                    <strong><i class="fa fa-times-circle" aria-hidden="true"></i> {{ $message }}</strong>
                  </span>
              ...
              <div class="row">
                <div class="col-md-6 form-group">
                  <a href="{{ route('culturehalls.list') }}" class="btn btn-lg btn-dark waves-effect waves-light"><i class="fe-chevron-left"></i> ย้อนกลับ </a>
                  <button type="button" class="btn btn-lg btn-danger waves-effect waves-light" id="close" data-id="{{ $data['culturehalls_find']->id }}">
                    <i class="mdi mdi-delete"></i> ยกเลิกข้อมูล
                  </button>
                </div>
                <div class="col-md-6 form-group text-right">
                  <a href="{{ route('culturehalls.dropzone', $data['culturehalls_find']->id) }}" class="btn btn-lg btn-secondary waves-effect waves-light">
                    <i class="fe-image"></i>
...
<script>
  $(document).on('click', '#close-pdf', function(event) {
    ...
    Swal.fire({
      ...
      if (result.value) {
        $.post("{{ route('close.culturehallspdf') }}", {
            _token: "{{ csrf_token() }}",
            id: id,
            file: file,
          })
    ...
    Swal.fire({
      ...
      if (result.value) {
        $.post("{{ route('close.culturehalls') }}", {
          ...
          .done(function(data, status, error) {
            if (error.status == 200) {
              location.href = "{{ route('culturehalls.list') }}";
            }
          })
          .fail(function(xhr, status, error) {
            //alert('An error occurred, please try again.');
