...
<div class="pd--80-0-20">
    <div class="container">
            ...
            <div class="col-md-12">
                <div class="top-title">
                    <div class="sec-title">
                        ...
                        <div class="h3">รายการแหล่งเรียนรู้ 3 บุรี</div>
                    </div>
                    <div>
                        <form class="header--search navbar-form" action="{{ route('learningslist') }}" method="GET">
                           <div style="display: flex;">
                                <input type="search" name="search" placeholder="ค้นหาเพิ่มเติม..." class="form-control m-0" value="{{ $data['search'] ?? '' }}" style="margin: 0 1px;">
                                <input type="number" name="year" placeholder="ค้นหาปีแหล่งเรียนรู้ 3 บุรี พศ." class="form-control m-0" value="{{ $data['year'] ?? '' }}" style="margin: 0 1px;">
                           </div>
        ...
        <div class="row">
         <div class="col-md-12">
                <div class="portfolio--filter-menu mt-2">
                    <ul class="nav">
                        <li class="active">
                            <a href="{{ route('learningslist') }}">ทั้งหมด</a>
                        </li>
                        @if(isset($data['Query_learningsType']) && count($data['Query_learningsType'])>0)
                        @foreach($data['Query_learningsType'] as $row)
                        <li>
                            <a href="{{ url('/learningslist?type='.$row->id) }}">
                                {{ $row->name }}
                            </a>
                        </li>
                        @endforeach
                ...
                <a href="{{ route('learnings', [$row->id]) }}">
                    <div class="card mb-2">
                        ...
                        <div class="card-body">
                            ...
                            <div class="font-size-11">แหล่งเรียนรู้ {{ $row->rLearningTypes->name ?? '-' }} </div>
            ...
            @endforeach
            <div class="col-md-12">
                <div style="float: inline-end;">{!! $data['Query_learnings']->links() !!}</div>
            </div>
            @else
